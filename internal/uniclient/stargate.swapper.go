...
func NewStargateSwapper(network v1.Network, c *BaseClientConfig) (defi.StargateSwapper, error) {

	proxy, err := socks5.NewSock5ProxyString(c.ProxyString, c.UserAgentHeader)
	if err != nil {
		return nil, errors.Wrap(err, "socks5.NewSock5ProxyString")
	}

	var cli defi.StargateSwapper
	switch network {
