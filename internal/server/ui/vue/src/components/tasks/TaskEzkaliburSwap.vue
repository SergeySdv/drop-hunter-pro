<template>
  <v-card-actions>
    <v-container>
      <div class="mb-3">use<a :href="taskProps.EzkaliburSwap.service.link"
                              target="_blank">{{ taskProps.EzkaliburSwap.service.name }}</a> to see
        available swap
        options
      </div>
      <v-row>
      ...
      <v-row>
        ...
        <v-col>
          <v-col>Slippage: 0.1% + protocol fee 0.25%</v-col>
        </v-col>
      </v-row>
    </v-container>
  </v-card-actions>
...
<script lang="ts">
import {EzkaliburSwapTask, MaverickSwapTask, Network, SpaceFiSwapTask, Task, TaskType, Token} from "@/generated/flow";
