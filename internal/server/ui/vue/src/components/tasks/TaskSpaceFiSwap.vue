<template>
  <v-card-actions>
    <v-container>
      <div class="mb-3">use<a :href="taskProps.SpaceFISwap.service.link"
                              target="_blank">{{ taskProps.SpaceFISwap.service.name }}</a> to see
        available swap
        options
      </div>
      <v-row>
...
<script lang="ts">
import {MaverickSwapTask, Network, SpaceFiSwapTask, Task, TaskType, Token} from "@/generated/flow";
...
export default defineComponent({
  ...
  data() {
    return {
      pairs: [
        ...
        tokenSwapPair(Token.ETH, Token.SPACE),
        tokenSwapPair(Token.SPACE, Token.ETH),
  ...
  methods: {
    required,
    getTask(): Task {
      const taskType = TaskType.SpaceFISwap
      const task = {
        weight: this.weight.toString(),
        spaceFiSwapTask: this.item,
        taskType: taskType,
        description: "",
      }
