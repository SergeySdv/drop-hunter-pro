<template>
  <v-card-actions>
    <v-container>
      <div class="mb-3">use<a :href="taskProps.ZkSwap.service.link"
                              target="_blank">{{ taskProps.ZkSwap.service.name }}</a> to see
        available swap
        options
      </div>
      <v-row>
...
export default defineComponent({
  ...
  methods: {
    required,
    getTask(): Task {
      const taskType = TaskType.ZkSwap
      const task = {
        weight: this.weight.toString(),
        zkSwapTask: this.item,
        taskType: taskType,
        description: "",
      }
