        ...
        <v-card-text>
          <v-container>
            <v-form validate-on="submit" ref="form">
              ...
              <div><b>Okex account</b> (optional) might be used for withdraw crypto from metamsk to okex</div>
              <br/>
              <v-row>
                <v-col>
                  <v-text-field
                    label="okex account/sub-account name"
                    v-model="okexAccount.subAccName"
                    density="comfortable"
                    variant="outlined"
                  ></v-text-field>
                </v-col>
                <v-col>
                  <v-text-field
                    label="okex deposit addr"
                    v-model="okexAccount.depositAddr"
                    density="comfortable"
                    variant="outlined"
                  ></v-text-field>
                </v-col>
  ...
  methods: {
    ...
    async loadProfile() {
      try {
        ...
        if (!this.item.okexAccount) {
          this.item.okexAccount = {
            subAccName: "",
            depositAddr: ''
          }
        } else {
          this.okexAccount = this.item.okexAccount
        }
      } catch (e) {
