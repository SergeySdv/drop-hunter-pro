...
func (t *EzkaliburSwapTask) Stop() error {
	t.cancel()
	return nil
}

func (t *EzkaliburSwapTask) Reset(ctx context.Context, a *Input) error {
	task := a.Task
...
func (t *EzkaliburSwapTask) Run(ctx context.Context, a *Input) (*v1.ProcessTask, error) {
	...
	if !ok {
		return nil, errors.New("panic.a.Task.Task.Task.(*v1.Task_EzkaliburSwapTask) call an ambulance!")
	}

	p := l.EzkaliburSwapTask
...
func EzkaliburSwap(ctx context.Context, profile *halp.Profile, p *v1.EzkaliburSwapTask, client zksyncera.EzkaliburSwapper, estimation *v1.EstimationTx) (*defi.DefaultSwapRes, *defi.Gas, error) {
	...
	res, err := client.EzkaliburSwap(ctx, &defi.DefaultSwapReq{
...
func EstimateEzkaliburSwapCost(ctx context.Context, profile *halp.Profile, p *v1.EzkaliburSwapTask, client zksyncera.EzkaliburSwapper) (*v1.EstimationTx, error) {
	res, _, err := EzkaliburSwap(ctx, profile, p, client, nil)
	if err != nil {
		return nil, err
	}
