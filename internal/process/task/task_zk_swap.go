...
func (t *ZkSwapTask) Run(ctx context.Context, a *Input) (*v1.ProcessTask, error) {
	...
	if !ok {
		return nil, errors.New("panic.a.Task.Task.Task.(*v1.Task_ZkSwapTask) call an ambulance!")
	}
	...
	if p.GetTx().GetTxId() == "" {
		...
		res, gas, err := ZkSwap(taskContext, profile, p, client, estimation)
		if err != nil {
			return nil, errors.Wrap(err, "ZkSwap")
		}
...
func ZkSwap(ctx context.Context, profile *halp.Profile, p *v1.ZkSwapTask, client zksyncera.ZkSwapper, estimation *v1.EstimationTx) (*defi.DefaultSwapRes, *defi.Gas, error) {
	...
	res, err := client.ZkSwap(ctx, &defi.DefaultSwapReq{
...
func EstimateZkSwapCost(ctx context.Context, profile *halp.Profile, p *v1.ZkSwapTask, client zksyncera.ZkSwapper) (*v1.EstimationTx, error) {
	res, _, err := ZkSwap(ctx, profile, p, client, nil)
	if err != nil {
		return nil, err
	}
